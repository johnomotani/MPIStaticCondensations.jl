...
function add_row_inds!(rv, idim, dimensions, block_sizes, nblock_list, row_indices,
                       ...
                       include_dense_boundaries, outer_dims_diagonal=true)
    @inbounds begin
            ...
            if idim ∈ row_dimension_indices && idim ∉ column_dimension_indices
                ...
                for row_inner ∈ 1:block_npoints + 1
                    add_row_inds!(rv, idim - 1, dimensions, block_sizes, nblock_list,
                                  ...
                                  outer_dims_diagonal && row_inner == iinner)
                end
                return nothing
            else
                row_inner = iinner
        ...
        if iinner == 1 && iblock > 1
            ...
            # This branch cannot ever be the 'outer_dims_diagonal'.
        ...
        row_offset = (iblock - 1) * block_npoints
        if include_dense_boundaries && outer_dims_diagonal && d.dense_boundaries && is_first_point
        ...
        if include_dense_boundaries && outer_dims_diagonal && d.dense_boundaries && is_last_point
            row_end = dn - 1
        else
            row_end = dn
        end
        if include_dense_boundaries && outer_dims_diagonal && iblock > nblock && d.dense_boundaries
        ...
        elseif iblock > nblock
            ...
            add_row_inds!(rv, idim - 1, dimensions, block_sizes, nblock_list, row_indices,
                          ...
                          row_dimension_indices, column_dimension_indices,
                          include_dense_boundaries, outer_dims_diagonal)
        else
    ...
    @inbounds begin
        ...
        if shared_comm_rank == 0
            ...
            for (icol, col) ∈ enumerate(column_indices)
                ...
                if !point_stencil && !include_dense_boundaries && col_eq ∈ keys(rv_lookup)
