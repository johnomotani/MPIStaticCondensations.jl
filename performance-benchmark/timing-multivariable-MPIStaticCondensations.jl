...
include("benchmark-multivariable-MPIStaticCondensations.jl")
...
function timing_run()
    ...
    if MPI.Comm_rank(MPI.COMM_WORLD) == 0
        save_timer("timing-multivariable-$comm_size-$n_shared.jld", timer)
        display(timer)
        #display(TimerOutputs.flatten(timer))
    end
    open("timing-multivariable-proc$comm_rank.txt", "w") do io
        show(io, timer)
        println(io)
    end
