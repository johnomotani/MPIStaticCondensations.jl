...
function run_MUMPS(x, matrix, data, global_i, global_j, local_i, local_j, rhs, rhs_global,
    ...
    total_size = Ref(max(maximum(global_i), maximum(global_j)))
    MPI.Allreduce!(total_size, max, comm)
